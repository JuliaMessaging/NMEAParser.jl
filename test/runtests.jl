...
@testset verbose = true "Test SPS data" begin
    ...
    @testset "Standard GPS" begin
        example = NMEAParser.nmea_parse(raw"$GPGGA,134740.000,5540.3248,N,01231.2992,E,1,09,0.9,20.2,M,41.5,M,,0000*61")
        @test(example.latitude ≈ 55.67208)
    end

    @testset "Standard Combined" begin
        example = NMEAParser.nmea_parse("\$GNGGA,033158.447,4532.5164,N,12258.1463,W,1,9,1.25,161.5,M,-19.6,M,,*7E")
        @test(example.longitude ≈ -122.969105)
    end
end

@testset "Test (GSA) GNSS DOP and Active Satellites" begin
    example = NMEAParser.nmea_parse(raw"$GPGSA,A,3,01,02,03,04,05,06,07,08,09,10,11,12,1.0,1.0,1.0*30")
    @test example.current_mode === 3
end

@testset "Test (RMC) Recommended Minimum Specific GNSS Data" begin
    example = NMEAParser.nmea_parse(raw"$GPRMC,154922.720,A,5209.731,N,00600.238,E,001.9,059.8,040123,000.0,W*7A")
...
@testset verbose=true "Test Proprietary Relative Position and Orientation types" begin
    @testset "PASHR" begin
        example = NMEAParser.nmea_parse(raw"$PASHR,154155.50,153.17,T,9.68,2.29,-0.07,0.502,0.502,0.959,1,0*19")
        @test example.heading == 153.17
        example = NMEAParser.nmea_parse(raw"$PASHR,154156.00,152.12,T,9.52,2.05,-0.06,0.504,0.504,0.956,1,0*12")
        @test example.heading_type
        example = NMEAParser.nmea_parse(raw"$PASHR,154156.10,151.86,T,9.46,1.98,-0.06,0.507,0.507,0.955,1,0*1C")
        @test example.ins_code == 0
    end
    @testset verbose=true "WPS based Position" begin
        @testset "PTWPOS" begin
            example = NMEAParser.nmea_parse(raw"$PTWPOS,154922.69,0.7,M,0.7,M,0,M,0.989949,M,0.01,K,F*07")
            @test example.valid
            example = NMEAParser.nmea_parse(raw"$PTWPOS,154922.7,0.8,M,0.8,M,0,M,1.131371,M,0.01,K,F*34")
            @test example.valid

            example = NMEAParser.nmea_parse(raw"$PTWPOS,021539.44,00000001.770,M,00000000.870,M,0000.000,M,0007.620,M,000.011281,K,F*88", validate_checksum=false)
            @test example.xpose === 1.77
            example = NMEAParser.nmea_parse(raw"$PTWPOS,021539.63,00000001.772,M,00000000.870,M,0000.000,M,0007.623,M,000.045134,K,F*fe", validate_checksum=false)
            @test example.ypose === 0.87
            example = NMEAParser.nmea_parse(raw"$PTWPOS,021539.72,00000001.774,M,00000000.870,M,0000.000,M,0007.624,M,000.067707,K,F*17", validate_checksum=false)
            @test example.direction === 'F'
            example = NMEAParser.nmea_parse(raw"$PTWPOS,021539.82,00000001.775,M,00000000.870,M,0000.000,M,0007.626,M,000.045134,K,F*dd", validate_checksum=false)
            @test example.time === 8139.82
            example = NMEAParser.nmea_parse(raw"$PTWPOS,021539.91,00000001.776,M,00000000.870,M,0000.000,M,0007.627,M,000.045134,K,F*60", validate_checksum=false)
            @test example.distance === 7.627
            example = NMEAParser.nmea_parse(raw"$PTWPOS,021540.01,00000001.778,M,00000000.871,M,0000.000,M,0007.628,M,000.045134,K,F*e9", validate_checksum=false)
            @test example.velocity ≈ 0.012537222222222222

            bad_pos = "\$TWPOS,154922.71,0.9,H,0.9,J,0,K,1.272792,M,0.01,M,F*39"
            @test_throws ArgumentError NMEAParser.nmea_parse(bad_pos, validate_checksum=false)
            bad_vel = "\$TWPOS,154922.72,1.0,K,1.0,K,0,K,1.414214,K,0.01,Q,F*3a"
            @test_throws ArgumentError NMEAParser.nmea_parse(bad_vel, validate_checksum=false)
        end
        @testset "PTWVCT" begin
            example = NMEAParser.nmea_parse(raw"$PTWVCT,154932.45,0.7,M,0.7,R,0.4,M,0.989949,M*3e")
            @test example.valid
        end
        ...
        @testset "PTWWHE" begin
            example = NMEAParser.nmea_parse(raw"$PTWWHE,021539.62,12,0.3,M,F,12,0.3,M,F,0.01*30")
            @test example.valid
        end
    end
    ...
    @testset verbose=true "IMU based orientation" begin
        @testset "TWHPR" begin
            example = NMEAParser.nmea_parse(raw"$PTWHPR,161540.45,12.456,78.901,2.34,79.912,0.12*2E")
            ...
            example_invalid = NMEAParser.nmea_parse(raw"$PTWHPR,161540.45,12.456,78.901,2.34,79.912,0.12*2C")
            @test example_invalid.valid === false

            example_default = NMEAParser.nmea_parse(raw"$PTWHPR,161540.45,12.456,78.901,2.34,79.912,0.12*2C", validate_checksum=false)
